...
class SatellitesProblem :
    ...
    Attributs :
        ...
        cities_coordinates (nparray(nparray(float, float, float))) : liste contenant les coordonnées des villes sur la terre. Si dimension == 2, des coordonnées cartésiennes (x,y,0) (en km) sont attendues. Si dimension == 3, les coordonnées géographiques (R,phi,lambda) avec -90° <= phi <= 90° (latitude) et -180° <= lambda <= 180° (longitude) sont attendues.
        cities_weights     (nparray(float))                        : liste de même taille que cities_coordinates contenant les poids des villes. Par défaut, nous supposons que ce poids correspond au nombre d'habitants de la ville (sinon, I_necessary doit être modifié).
        ...
        alpha (float) : paramètre pour rendre continue la fonction objectif
        gamma (float) : paramètre pour rendre continue la fonction objectif

    Méthodes :
        __init__(self, dimension, R, H, P, I_necessary)
        input_from_file(self, file_name, N_satellites)
        __distance(self, index_city, index_sat, continuous=False)
        __cost_city(self, index_city)
        cost(self)
        __grad_2D(self)
        __grad_3D(self)
        grad(self)
    ...
    def input_from_file(self, file_name, N_satellites) :
        ...
        self.I_necessary a été modifié pour correspondre au nombre d'habitants d'une ville ayant un poids de 1.
        """
    ...
    def __square_distance(self, index_city, index_sat) :
        ...
        if   self.dimension == 2 : return np.linalg.norm(self.cities_coordinates[index_city] - self.sat_coordinates[index_sat])**2
            ...
            """if   not continuous and distance > self.max_square_distance              : return np.inf
            elif     continuous and distance > self.alpha * self.max_square_distance : return distance + (distance - self.alpha * self.max_square_distance)**self.gamma
