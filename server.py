...
def compute():
    ...
    cities_gps = np.array(cities_gps).astype(np.float64)

    cities_spherical = gps2spher(np.array(cities_gps).astype(np.float64))
    ...
    satellites_gps = spher2gps(satellites_polar)
    print(satellites_gps)
    plot_map.create_fig()
    plot_map.plot_cities(*cities_gps.T, cities_weights)
    plot_map.plot_satellite_long_lat(*satellites_gps.T)
